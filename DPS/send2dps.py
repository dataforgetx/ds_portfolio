...
def get_filtered_data_for_period(fiscal_year, quarter, envconfig):
    """
    Query database and filter data based on fiscal year and quarter.
    
    Args:
        fiscal_year (int): Fiscal year (e.g., 2025)
        quarter (int or None): Quarter number (1-4) or None for full fiscal year
        envconfig (dict): Environment configuration dictionary
        
    Returns:
        DataFrame: Filtered dataframe
    """
    # Build query with WHERE clause to filter at database level
    connection = None
    try:
        ...
        if quarter is None:
            # Full fiscal year
            query = f"SELECT * FROM caps.qtr_dps_cvs WHERE RPT_FISCAL_YR = {fiscal_year}"
        else:
            # Specific quarter
            query = f"SELECT * FROM caps.qtr_dps_cvs WHERE RPT_FISCAL_YR = {fiscal_year} AND RPT_QTR = {quarter}"
        
        filtered_data = pd.read_sql(query, connection)
        log.info(f"Filtered rows for fiscal_year={fiscal_year}, quarter={quarter}: {len(filtered_data)}")
        
        return filtered_data
    except Exception as e:
        log.error(f"Failed to query filtered data for fiscal_year={fiscal_year}, quarter={quarter}: {e}")
    ...
    # else:
    #     filtered_data = df[(df['RPT_FISCAL_YR'] == fiscal_year) & (df['RPT_QTR'] == quarter)]
...
def split_name_by_delimiter(df, col_name, delimiter, max_parts, exclude_list=None, remove_delimiter=True):
    ...
    if remove_delimiter:
        result[col_name] = result[col_name].str.replace(delimiter, '', regex=False)
...
def main():
    ...
    try:
        # Get current date
        run_date = date.today()
        # run_date = date(2025, 9, 20)  # testing

        # Get fiscal year, quarter, and query_period
        fiscal_year, quarter, query_period = get_query_period_util(run_date)
        log.info(f"Query period: {query_period} (fiscal_year={fiscal_year}, quarter={quarter})")

        # Load data into caps.qtr_dps_cvs table by calling stored procedure
        connection = None
        try:
            connection = get_dbconnection(envconfig)
            cursor = connection.cursor()
            log.info(f"Calling stored procedure qtr_dps_cvs_load for fiscal_year={fiscal_year}, quarter={quarter}")
            sql = "BEGIN caps.qtr_dps_cvs_pkg.qtr_dps_cvs_load(:rpt_fiscal_yr, :rpt_qtr); END;"
            cursor.execute(sql, {'rpt_fiscal_yr': fiscal_year, 'rpt_qtr': quarter})
            connection.commit()
            log.info("Successfully loaded data into caps.qtr_dps_cvs table")
        except Exception as e:
            log.error(f"Failed to load data into caps.qtr_dps_cvs table: {e}")
            log.error(f"Error details: {traceback.format_exc()}")
            if connection:
                connection.rollback()
            raise
        ...
        filtered_data = get_filtered_data_for_period(fiscal_year, quarter, envconfig)
        ...
        if not sftp_upload_success:
            try:
                ...
                send_notification_email(recipient_emails, subject, message, logger=log)
            except Exception as e:
                log.warning(f"Failed to send SFTP error notification email: {e}")
        ...
        remote_directory = f"{remote_dir_head}/TO_DPS"
        ...
        # Archive both files (do not archive, archive at the end of step 2)
        ...
        email_error(message, error_emails, "send2dps.py", f"{log_dir}/send2dps.log")
        log.error("=" * 80)
        raise
